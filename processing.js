...
function predictImage() {
    ...
    let dim = new cv.Size(width, height);
    
    cv.resize(image, image, dim, 0, 0, interpolation=cv.INTER_AREA);
    ...
    cv.copyMakeBorder(image, image, topPadding, bottomPadding, leftPadding, rightPadding, cv.BORDER_CONSTANT, black); 
    ...
    const yShift = Math.round(image.cols/2.0 - cy);

    newSize = new cv.Size(image.cols, image.rows);
    const M = cv.matFromArray(2, 3, cv.CV_64FC1, [1,0,xShift,0,1,yShift]);

    cv.warpAffine(image, image, M, newSize, cv.INTER_LINEAR, cv.BORDER_TRANSPARENT, black);
    ...
    // const outputConvas = document.createElement('CANVAS');
    // cv.imshow(outputConvas, image)
    // document.body.appendChild(outputConvas)
